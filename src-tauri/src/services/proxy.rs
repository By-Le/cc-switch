...
impl ProxyService {
    ...
    fn write_codex_takeover_live_for_provider(
        ...
        if crate::settings::preserve_codex_official_auth_on_switch() {
                ...
                .filter(|auth| Self::codex_auth_has_proxy_placeholder(auth))
            {
                ...
                let (prepared_config, catalog_rollback) =
                    crate::codex_config::prepare_codex_live_config_text_with_optional_catalog_rollback(
                        config, config_str, profile,
                    )
                    .map_err(|e| format!("写入 Codex 配置失败: {e}"))?;
                let result =
                    crate::codex_config::prepare_codex_provider_live_config(auth, &prepared_config)
                        .and_then(|live_config| {
                            crate::codex_config::write_codex_live_config_atomic(Some(&live_config))
                        });
                if let Err(e) = result {
                    if let Some(rollback) = catalog_rollback {
                        rollback.rollback();
                    }
                    return Err(format!("写入 Codex 配置失败: {e}"));
                }
                return Ok(());
            }
        }
    ...
    fn write_codex_live_verbatim(&self, config: &Value) -> Result<(), String> {
        ...
        // Verbatim restore has no Provider in hand, so use ProxyChat until a
        // provider switch rewrites the live snapshot with its exact profile.
        let (prepared_cfg, catalog_rollback) = match config_str {
            Some(cfg) => {
            ...
            match (auth, prepared_cfg.as_deref()) {
                (Some(auth), Some(cfg)) => {
                    ...
                    if auth.as_object().is_some_and(|obj| obj.is_empty()) {
                        let old_auth = if auth_path.exists() {
                            Some(
                                std::fs::read(&auth_path)
                                    .map_err(|e| format!("读取 Codex auth 备份失败: {e}"))?,
                            )
                        } else {
                            None
                        };
                        ...
                        if let Err(e) = crate::config::write_text_file(&config_path, cfg) {
                            if let Some(bytes) = old_auth {
                                let _ = crate::config::atomic_write(&auth_path, &bytes);
                            }
                            return Err(format!("写入 Codex config 失败: {e}"));
                        }
                    } else {
...
"#;
    ...
    #[test]
    #[serial]
    fn codex_provider_live_rolls_back_catalog_when_config_write_fails() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        seed_codex_model_template();
...
"#;
        ...
        let catalog_path = crate::codex_config::get_codex_model_catalog_path();
        let old_catalog = std::fs::read_to_string(&catalog_path).expect("read provider A catalog");
        assert!(old_catalog.contains(r#""slug": "model-a""#));
        assert!(!old_catalog.contains(r#""slug": "model-b""#));

        let config_path = crate::codex_config::get_codex_config_path();
        std::fs::remove_file(&config_path).expect("remove config.toml");
        std::fs::create_dir_all(&config_path).expect("force config write failure");

        let err = service
            .write_codex_live_for_provider(&provider_b.settings_config, Some(&provider_b))
            .expect_err("provider B config write should fail");

        assert!(
            err.contains("写入 Codex 配置失败"),
            "unexpected error: {err}"
        );
        let restored_catalog =
            std::fs::read_to_string(&catalog_path).expect("read restored catalog");
        assert_eq!(
            restored_catalog, old_catalog,
            "failed config write must roll back cc-switch-model-catalog.json"
        );
    }

    #[test]
    #[serial]
    fn codex_empty_auth_restore_keeps_existing_auth_when_config_write_fails() {
        let _home = TempHome::new();
        crate::settings::reload_settings().expect("reload settings");
        seed_codex_model_template();
        ...
        let old_auth = json!({
            "tokens": {
                "id_token": "old-oauth-token"
            }
        });
        crate::config::write_json_file(&auth_path, &old_auth).expect("seed auth");
        let catalog_path = crate::codex_config::get_codex_model_catalog_path();
        let old_catalog = r#"{"models":[{"slug":"model-a"}]}"#;
        std::fs::write(&catalog_path, old_catalog).expect("seed catalog");
        ...
        let err = service
            ...
            .expect_err("config write failure should be surfaced");

        assert!(
            err.contains("写入 Codex config 失败"),
            "unexpected error: {err}"
        );
        let restored_auth: Value =
            crate::config::read_json_file(&auth_path).expect("auth should remain readable");
        assert_eq!(
            restored_auth, old_auth,
            "failed config write must not delete the previous Codex auth.json"
        );
        let restored_catalog =
            std::fs::read_to_string(&catalog_path).expect("catalog should remain readable");
        assert_eq!(
            restored_catalog, old_catalog,
            "failed config write must not replace the previous cc-switch-model-catalog.json"
        );
    }
