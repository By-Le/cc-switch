...
pub(crate) struct CodexModelCatalogRollback {
    path: PathBuf,
    old_contents: Option<Vec<u8>>,
}

impl CodexModelCatalogRollback {
    fn capture(path: PathBuf) -> Result<Self, AppError> {
        ...
        Ok(Self { path, old_contents })
    }

    pub(crate) fn rollback(self) {
        match self.old_contents {
            Some(bytes) => {
                let _ = atomic_write(&self.path, &bytes);
            }
            None => {
                let _ = delete_file(&self.path);
            }
        }
    }
}
...
/// Generate Codex `model_catalog_json` from provider settings, inject/remove
/// its top-level TOML fields, and return a rollback handle for follow-up writes.
pub(crate) fn prepare_codex_config_text_with_model_catalog_rollback(
    settings: &Value,
    config_text: &str,
    profile: CodexCatalogToolProfile,
) -> Result<(String, Option<CodexModelCatalogRollback>), AppError> {
    ...
    if let Some(catalog) = codex_model_catalog_from_settings(settings, config_text, profile)? {
        ...
        let rollback = CodexModelCatalogRollback::capture(catalog_path.clone())?;
        if let Err(e) = write_json_file(&catalog_path, &catalog) {
            rollback.rollback();
            return Err(e);
        }
        Ok((config_text, Some(rollback)))
    } else {
...
/// Read the cc-switch-maintained catalog file referenced by
...
pub(crate) fn prepare_codex_live_config_text_with_optional_catalog_rollback(
    settings: &Value,
    config_text: &str,
    profile: CodexCatalogToolProfile,
) -> Result<(String, Option<CodexModelCatalogRollback>), AppError> {
    if settings.get("modelCatalog").is_some() {
        prepare_codex_config_text_with_model_catalog_rollback(settings, config_text, profile)
    } else {
        Ok((config_text.to_string(), None))
    }
}
...
pub fn write_codex_provider_live_with_catalog(
    ...
    let (prepared_config, catalog_rollback) = match config_text {
        Some(text) => {
            let (prepared, rollback) =
                prepare_codex_config_text_with_model_catalog_rollback(settings, text, profile)?;
            (Some(prepared), rollback)
        }
        None => (None, None),
    };

    if let Err(e) = write_codex_live_for_provider(category, auth, prepared_config.as_deref()) {
