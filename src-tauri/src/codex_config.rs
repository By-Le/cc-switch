...
/// the top-level TOML field, and return a rollback handle for follow-up writes.
pub(crate) fn prepare_codex_config_text_with_model_catalog_rollback(
    ...
    if let Some(catalog) = codex_model_catalog_from_settings(settings, config_text)? {
        ...
        Ok((set_codex_model_catalog_json_field(config_text, None)?, None))
    }
}
...
pub(crate) fn prepare_codex_live_config_text_with_optional_catalog_rollback(
    ...
    if settings.get("modelCatalog").is_some() {
        prepare_codex_config_text_with_model_catalog_rollback(settings, config_text)
    } else {
    ...
    let (prepared_config, catalog_rollback) = match config_text {
        Some(text) => {
            let (prepared, rollback) =
                prepare_codex_config_text_with_model_catalog_rollback(settings, text)?;
