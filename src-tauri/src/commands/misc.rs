...
mod tests {
    ...
    mod anchored_upgrade_windows {
        ...
        fn volta_windows_uses_volta_install() {
            ...
            // Codex 不走 `codex update`：该命令在 npm/平台分发损坏时会假成功，Windows
            // 也应直接锚定包管理器。
        ...
        #[test]
        fn windows_no_sibling_returns_none_for_codex() {
            // Codex 不再跑官方 self-update；同目录没有可锚定包管理器时返回 None，
            // 由上层 static fallback 处理。
